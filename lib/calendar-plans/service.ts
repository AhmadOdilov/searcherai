...
export async function regenerateCalendarPlan(
  ...
  */
  const claimed = await prisma.calendarPlan.updateMany({
  ...
  /*
    Eski faylni band qilingandan KEYIN o'chiramiz — faqat poygada
    YUTGAN so'rov faylga tegadi.
  */
  if (existing.filePath !== null) {
    await deleteFile("xlsx", existing.filePath);
  }

  const reset = await prisma.calendarPlan.findFirstOrThrow({
    where: { id, userId },
    select: DETAIL_FIELDS,
  });
